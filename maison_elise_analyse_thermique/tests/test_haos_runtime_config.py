...
def test_old_options_remain_compatible_with_new_optional_defaults() -> None:
    ...
    assert env["THERMAL_MAIL_ENTITY"] == DEFAULT_MAIL_ENTITY


def test_explicit_weather_notification_and_mail_targets_are_forwarded_to_runtime() -> None:
    options = _base_options()
    options["weather_entity"] = "weather.dammarie_les_lys"
    options["notification_service"] = "notify.mobile_app_phone"
    options["mail_entity"] = "notify.maison_cognitive_email"
    ...
    assert env["THERMAL_MAIL_ENTITY"] == "notify.maison_cognitive_email"
